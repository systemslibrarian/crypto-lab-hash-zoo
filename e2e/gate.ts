 ...
 *     this file replaces force-opened every <details>, added `.is-active` to all
 *     four Info Panel tab panels at once, and stripped inline `display:none` off
 *     every element — a state no visitor can reach, since the tablist shows
 *     exactly one panel at a time. It also froze `animation` and `transition` to
 *     `none`, which makes a suite structurally unable to observe a transition or
 *     theme-swap defect. Every panel is reachable by clicking its tab, and this
 *     gate reaches them that way instead; motion is settled honestly (see
 *     `settle`) rather than frozen.
 *
 ...
 *     result surfaces — the six-digest comparison table, the avalanche bit
 *     grids, the distribution readout and the length-extension forgery — are all
 *     downstream of a click and render nothing at all until then.
 *
 ...
 * that injects `opacity: 1` (as the replaced gate did) paints it back for the
 * scanner alone. The avalanche bit-cells fade in per-cell, which is exactly the
 * shape of thing that can strand at zero.
 */
...
export async function boot(page: Page, theme: 'dark' | 'light'): Promise<void> {
  ...
  await expect(page.locator('#app main .panel')).toHaveCount(5);
  await expect(page.locator('#hash-intro h2')).toContainText('What is a hash?');
  await expect(page.locator('#info-panel [role="tab"]')).toHaveCount(4);
  // The intro digest is computed on load from the seeded input, so it is the
  // signal that the lab's hashing path has actually run rather than merely
  // rendered its markup.
  await expect(page.locator('#intro-hash')).not.toBeEmpty();
 ...
 * plausible offender: it renders six 64-hex digests side by side, a 256-cell
 * avalanche bit grid, and a monospace padding-block dump in the modal. Each
 * belongs inside its own `overflow-x: auto` container; one missed, or a long
 * digest outside one, pushes the whole document sideways at phone width.
 */
 ...
 * content actually overflows — the digest table and bit grids only overflow at
 * phone width, so a desktop-only gate never saw them. This assertion runs
 ...
 *
 * The replaced gate scanned two states: the untouched page (with all four tab
 * panels forced open at once) and the padding modal. Everything below the fold
 * of a click — the digest comparison, the avalanche grids, the distribution
 * readout, the length-extension forgery — rendered empty at scan time and was
 * therefore never checked.
 */
export async function driveAllStates(page: Page, theme: string): Promise<void> {
  await scan(page, `${theme} / first paint`);

  // Section A — six digests of one message, the widest content on the page.
  await page.locator('#message-input').fill('the quick brown fox');
  await page.locator('#hash-btn').click();
  await expect(page.locator('#hash-results .hex-wrap')).toHaveCount(3);
  await scan(page, `${theme} / digest comparison`);

  // Section B — avalanche grids, one flipped bit at a time.
  await page.locator('#bit-slider').fill('7');
  await page.locator('#analyze-btn').click();
  await expect(page.locator('#avalanche-grids .bit-cell').first()).toBeVisible();
  await scan(page, `${theme} / avalanche grids`);

  // Section C — the distribution readout.
  await page.locator('#dist-btn').click();
  await expect(page.locator('#dist-result')).not.toBeEmpty();
  await scan(page, `${theme} / distribution`);

  // Section C — the length-extension forgery, the one result that renders a
  // "this succeeded" verdict about a broken construction.
  await page.locator('#lext-secret').fill('sekret');
  await page.locator('#lext-append').fill('&admin=true');
  await page.locator('#lext-btn').click();
  await expect(page.locator('#lext-result')).not.toBeEmpty();
  await scan(page, `${theme} / length extension`);

  // Info Panel — one tab at a time, which is the only way a visitor sees them.
  for (const tab of ['tab-choose', 'tab-md', 'tab-sponge', 'tab-tree']) {
    await page.locator(`#${tab}`).click();
    await expect(page.locator(`#${tab}`)).toHaveAttribute('aria-selected', 'true');
    await scan(page, `${theme} / info panel: ${tab}`);
  }

  // The padding modal, scanned as the whole page rather than in isolation —
  // a modal's contrast depends on the backdrop it is composited over.
  await page.locator('#padding-btn').click();
  await expect(page.locator('#padding-modal')).toBeVisible();
  await scan(page, `${theme} / padding modal`);
